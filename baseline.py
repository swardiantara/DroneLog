...
parser.add_argument('--pooling', type=str, choices=['cls', 'max', 'min', 'mean'], default='mean',
...
def main():
    ...
    max_seq_length = 64
    batch_size = 8
    train_dataset = CustomDataset(train_df, tokenizer, max_seq_length)
    train_loader = DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True)

    test_dataset = CustomDataset(test_df, tokenizer, max_seq_length)
    ...
    if save_best_model:
        ...
        if not os.path.exists(best_model_dir):
            ...
            model_scripted.save(os.path.join(best_model_dir, 'pytorch_model.pt')) # Save
        else:
